  ...
  if (periods.length === 0) {
    return (
      <section className="rounded-2xl border border-[var(--border-card)] bg-[var(--surface-08)] backdrop-blur-xl">
        <p className="border-b border-[var(--border-divider)] px-4 py-2 text-[12px] text-[var(--text-body)]">
        ...
        {slots.map((slot) => {
          ...
          return (
            <div key={slot.key} className="flex flex-col items-center gap-2 px-2 py-4 text-center">
              <div className="text-xs font-medium text-[var(--text-body)]">{slot.label}</div>
              {isFair ? (
